...
import { Stock, Product, Factory, Depot, Truck } from "@/lib/models";
...
export async function GET(req: NextRequest) {
  ...
  const stock = await Stock.find(filter).populate("productId").sort({ createdAt: -1 }).lean();
  const populated = await Promise.all(stock.map(populateLocation));
