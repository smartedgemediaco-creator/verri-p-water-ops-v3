...
const STEPS: Step[] = [
  ...
  { id: "users", title: "Invite Staff", description: "Create accounts for factory managers, depot managers, and drivers with role-based access.", href: "/users/new", icon: "👤", order: 5, estimate: "3 min", check: async () => { const r = await fetch("/api/users"); const d = await r.json(); return Array.isArray(d) && d.length > 1; } },
  { id: "production", title: "Record Production", description: "Log production batches. This automatically adds stock to the factory's stock.", href: "/factories", icon: "⚙️", order: 6, estimate: "2 min", check: async () => { const r = await fetch("/api/stock/stats"); const d = await r.json(); return (d.totalProduced ?? 0) > 0; } },
  { id: "transfers", title: "Transfer Stock", description: "Move stock between factories, depots, and trucks. Track each transfer from dispatch to delivery.", href: "/transfers/new", icon: "🔄", order: 7, estimate: "3 min", check: async () => { const r = await fetch("/api/transfers"); const d = await r.json(); return Array.isArray(d) && d.length > 0; } },
  { id: "sales", title: "Record Sales", description: "Log sales at depots and factories. Stock is deducted automatically. Supports cash, POS, transfer, and credit.", href: "/sales/new", icon: "💰", order: 8, estimate: "2 min", check: async () => { const r = await fetch("/api/sales"); const d = await r.json(); return Array.isArray(d) && d.length > 0; } },
  { id: "costs", title: "Track Costs", description: "Record business expenses — fuel, maintenance, salaries. See profitability per location.", href: "/costs", icon: "📉", order: 9, estimate: "2 min", check: async () => { const r = await fetch("/api/costs"); const d = await r.json(); return Array.isArray(d) && d.length > 0; } },
  { id: "pos-devices", title: "Register POS Terminals", description: "Add Moniepoint, OPay, or PalmPay terminals. Link to locations so transactions auto-record as sales.", href: "/pos-devices", icon: "💳", order: 10, estimate: "3 min", check: async () => { const r = await fetch("/api/pos-devices"); const d = await r.json(); return Array.isArray(d) && d.length > 0; } },
];
