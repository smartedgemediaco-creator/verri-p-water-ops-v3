...
const STEPS: Step[] = [
  ...
  { id: "trucks", title: "Register Delivery Vehicles", description: "Add delivery trucks/tricycles with plate, chassis, and engine numbers. Assign drivers and link each vehicle to a factory or depot.", href: "/trucks/new", icon: "🚚", order: 4, estimate: "3 min", check: async () => { const r = await fetch("/api/trucks"); const d = await r.json(); return Array.isArray(d) && d.length > 0; } },
  ...
  { id: "production", title: "Record Production", description: "Log production batches. This automatically adds stock to the factory's stock.", href: "/production/new", icon: "⚙️", order: 7, estimate: "2 min", check: async () => { const r = await fetch("/api/stock/stats"); const d = await r.json(); return (d.totalProduced ?? 0) > 0; } },
