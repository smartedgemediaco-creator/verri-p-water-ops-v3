...
import { BoxIconLine, ListIcon, AlertIcon, TrashBinIcon, ChevronDownIcon, ChevronRightIcon } from "@/icons";
...
export default function InventoryPage() {
  ...
  useEffect(() => {
    ...
    const fetchAll = () => {
      ...
      Promise.all([
        ...
        fetch(`/api/inventory/stats?${params}`).then((r) => r.json()),
      ])
  ...
  const toggleExpand = async (itemId: string) => {
    ...
    try {
      const res = await fetch(`/api/inventory/${itemId}/activity`);
  ...
  useEffect(() => {
    if (!spoilForm.locationType) { setSpoilLocationOptions([]); return; }
    ...
    <div>
      ...
      {stats && filterLocation === "all" && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 px-3 py-2 shadow-theme-sm min-w-0">
        ...
        <div className="px-4 py-3 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
          <h3 className="text-sm font-semibold text-gray-800 dark:text-white/90">
            {filterLocationId && selectedLocName
              ? `Inventory at ${selectedLocName}`
              : filterLocation !== "all"
              ? `All ${filterLocation} Inventory Records`
        ...
        <Table>
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow>
                <TableCell className="text-center py-10 text-gray-500 dark:text-gray-400 text-sm" colSpan={6}>No inventory records match your filters.</TableCell>
              </TableRow>
            ) : (
              items.map((item) => {
                ...
                return (
                  <Fragment key={item._id}>
                    ...
                    >
                      ...
                      <TableCell className="py-3 text-theme-sm text-gray-800 dark:text-white/90">{item.locationName ?? item.locationId?.slice(-6) ?? "N/A"}</TableCell>
                    ...
                    {isOpen && (
                      <TableRow>
                        <TableCell colSpan={6} className="p-0 bg-gray-50 dark:bg-white/[0.02]">
                          {activityError ? (
                            ...
                            <div className="px-6 py-4 space-y-4">
                              ...
                              {!act.productions.length && !act.sales.length && !act.transfers.length && !act.wastages.length && (
                                <p className="text-sm text-gray-400">No recorded activity for this inventory item.</p>
                              )}
                            </div>
                          )}
                        </TableCell>
